...
for fold in range(5):
    ...
    train_pairs, test_pairs = prepare_data_for_bert(pairs_trained, pairs_tested)
    pdb.set_trace()
    # Initialize models
    ...
    for epoch in range(n_epochs):
        ...
        input_batches, input_lengths, output_batches, output_lengths, nums_batches, num_pos_batches, num_size_batches =prepare_train_batch_text(pairs_trained,batch_size)
        # pdb.set_trace()
        print("fold:", fold + 1)
        print("epoch:", epoch + 1)
        ...
        for idx in range(len(input_lengths)):
            ...
            '''
            loss_probing_compare, correct_list_batch = train_probing_opter(input_batches[idx], input_lengths[idx], output_batches[idx],output_lengths[idx], encoder, probing_opter_module, probing_opter_optim, nums_batches[idx], num_pos_batches[idx],output_lang)
            loss_total += loss_probing_compare
        ...
        input_batches, input_lengths, output_batches, output_lengths, nums_batches, num_stack_batches, num_pos_batches, num_size_batches = prepare_train_batch(test_pairs, batch_size)
        for idx in range(len(input_lengths)):
            loss_probing_compare, correct_list_batch = test_probing_opter(input_batches[idx], input_lengths[idx], output_batches[idx],output_lengths[idx], encoder, probing_opter_module, probing_opter_optim, nums_batches[idx], num_pos_batches[idx],output_lang)
            loss_total += loss_probing_compare
